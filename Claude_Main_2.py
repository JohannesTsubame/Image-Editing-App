...
from PIL import Image, ImageDraw, ImageFilter, ImageEnhance, ImageFont, ImageOps, ImageTk
import numpy as np
import os
import sys
import copy
...
class PhotoCraft(ctk.CTk):
    ...
    def render(self):
        flat = self._flatten()
        w = max(1, int(self.canvas_width * self.zoom))
        h = max(1, int(self.canvas_height * self.zoom))
        display = flat.resize((w, h), Image.LANCZOS if self.zoom < 1 else Image.NEAREST)

        # Must use ImageTk.PhotoImage for tk.Canvas — CTkImage is for CTk widgets only
        self._tk_image = ImageTk.PhotoImage(display)

        self.canvas.delete("image")
        cw = self.canvas.winfo_width()
        ch = self.canvas.winfo_height()
        cx = (cw if cw > 1 else 900) // 2 + self.canvas_offset[0]
        cy = (ch if ch > 1 else 650) // 2 + self.canvas_offset[1]
        self.canvas.create_image(cx, cy, image=self._tk_image, anchor="center", tags="image")
